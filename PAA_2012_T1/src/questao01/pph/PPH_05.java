	...
	// O nome do arquivo de input padrão(usado para testes).
		...
		private void run(String inputFile) {
			try {
				...
				//Ordanando a lista
				//for (int i = listOrderedPairs.size()-1; i >= 0; i--) {
			//		System.out.println(i + " - "+ listOrderedPairs.get(i).getA() + " " +listOrderedPairs.get(i).getB());	
				//}
				...
				for (int i = 0; i <= listOrderedPairs.size()-1; i++) {
						System.out.println(i + " - "+ listOrderedPairs.get(i).getRatio());	
					}
