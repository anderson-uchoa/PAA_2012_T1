...
public class PPH_02 {
   ...
   */
  private int[][] getValuesFromInputFile(int[][] arrayOrderedPairs, int index, int quantityOfInputValues, Scanner scanner) {
  ...
  private float maximumRatio(float previousRatio, int[][] arrayOrderedPairs) {
    ...
    for (int i = 1; i < arrayOrderedPairs.length; i++) {
      ...
      Log.debugF("[%d, %d] = %f - %f", arrayOrderedPairs[i][ColumnA], arrayOrderedPairs[i][ColumnB], auxRatio, currentRatio);
      Log.debug("");
