...
public class Frascos_04 {
  ...
  private boolean increment(boolean[] input, int startPos, int endPos) {
    ...
    for (i = length - 1; i >= 0; i--) {
      ...
      boolean temp = input[j];

      input[j] = ((temp ^ increment[i]) ^ c); //a xor b xor c

      c = ((temp & increment[i]) | (temp & c)) | (increment[i] & c); //ab+bc+ca
      j--;
    }
