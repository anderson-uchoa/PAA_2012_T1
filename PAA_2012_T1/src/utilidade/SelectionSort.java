...
public class SelectionSort {
	...
	private int partition(List<OrderedPair> list, int left, int right,
		...
		for (int i = left; i < right - 1; i++) {
			if (list.get(i).getRatio() <= pivotValue) {
				// Swapping
	...
	private OrderedPair select(List<OrderedPair> list, int left, int right,
			int k) {
		...
		int pivotNewIndex = partition(list, left, right,
				medianOfMedians(list, left, right));
	...
	private int selectIdx(List<OrderedPair> list, int left, int right, int pivot) {
		...
		if (size % 2 == 0)
			...
			idx = ((size + 1) / 2) + ((size + 1) / 2) + 1;
		}
		return list.indexOf(nova.get(idx));
	}

	private void InsertionSort(List<OrderedPair> list) {
